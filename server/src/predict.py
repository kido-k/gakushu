...
def main(gcp, predict_image_file, file_name, classes):
    ...
    X.append(data)
    X = np.array(X)
    model = build_model(gcp, file_name, classes)
...
def build_model(gcp, file_name, classes):
    model = Sequential()
    ...
    # モデルのロード
    blob = gcp['bucket'].blob('model/' + file_name + '.h5')
